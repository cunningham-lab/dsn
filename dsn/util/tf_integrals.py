...
gauss_points = np.expand_dims(np.expand_dims(np.expand_dims(gauss_points*np.sqrt(2), 0), 0), 0);
...
def Phi (mu, delta0):
    mu = tf.expand_dims(mu, 3);
    delta0 = tf.expand_dims(delta0, 3);
    integrand = tf.tanh(mu+tf.sqrt(delta0)*gauss_points)
    return gaussian_norm * tf.tensordot(integrand,gauss_weights, [3,0]);
...
def PhiSq (mu, delta0):
    ...
    delta0 = tf.expand_dims(delta0, 3);
    integrand = tf.tanh(mu+tf.sqrt(delta0)*gauss_points)
    return gaussian_norm * tf.tensordot(tf.square(integrand), gauss_weights, [3,0]);
