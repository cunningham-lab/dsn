...
def get_quadrature(num_pts, double=False):
    ...
    if (double):
        ...
        return gauss_norm, gauss_points, gauss_weights, \
               gauss_points_inner, gauss_points_outer
    else:
        return gauss_norm, gauss_points, gauss_weights
...
def IntPrimPrim (mu, delta0, deltainf, num_pts=200): # Performs the external integral
    gauss_norm, gauss_points, gauss_weights, gauss_points_inner, \
        gauss_points_outer = get_quadrature(num_pts, double=True)
    ...
    outer_integrand = gauss_norm * tf.tensordot(inner_integrand, gauss_weights, [[2],[0]])
