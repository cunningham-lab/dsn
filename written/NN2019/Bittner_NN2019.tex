...
Analytic approaches to studying nonlinear dynamical system are much more complicated when moving from two-dimensional to three- or four-dimensional systems in the absence of restrictive simplifying assumptions \cite{strogatz1994nonlinear}. 
...
There are many considerations when assessing the feasibility of classical statistical inference in neuroscience such as conjugacy, scalability to large data-sets, and likelihood tractability. EPI is no exception, and we emphasize that it is practicable in most settings in theoretical neuroscience.  When working with models of close biological realism with complex nonlinear dynamics, EPI is capable of learning distributions of model parameters producing low-level signatures of computation as we have shown through our analysis of the STG. This approach is not specific to models of such biological realism, as we have its utility studying RNNs, which are quite abstract.  We are able to condition both deterministic and stochastic models on all sorts of  emergent properties from membrane potential firing to execution of approximate inference, so long as these emergent property statistics are continuously differentiable with respect to the model parameters.

There are important technical considerations when considering the suitability of EPI for a particular modeling question.  First, one must consider how computationally expensive the gradient of the emergent propety statistic is with respect to the model parameters.  In the most suitable of cases, there is a simple, closed form expression (e.g. Section \ref{methods_2DLDS}).  On the other end of the spectrum, you may require many a large number simulation iterations before a high quality measurement of the emergent property statistic is available as in measuring firing frequency in the STG (e.g. Section \ref{methods_STG}).  In such cases, optimization will be expensive, and it is worth considering an alternative methodology (see Section \ref{methods_related_work}).   Secondly, the defined emergent property should always be appropriately conditioned.  Of course, learning can not occur when under- or overconstrained.  When underconstrained, the posterior grows (in entropy) unstabily unless mapped to a finite support.  If overconstrained, and there is no support producing the emergent property, EPI optimization will never converge.

\section{Discussion}
\textbf{Draft in progress}: \\ 
\textbf{Big picture}
\begin{itemize}
\item Machine learning has played an effective, multifaceted role in neuroscientific progress.
\item Primarily, it has been used to reveal structure in large scale neural data sets \cite{kass2001spike, brown1998statistical, paninski2004maximum, byron2009gaussian, latimer2015single, duncker2019learning} (see review, \cite{paninski2018neural}).
\item Secondarily, trained algorithms of varying degrees of biological similarity serve as fully-observable computational systems, that are compared to the brain.
\item Theoretical neuroscientists have focused more heavily on this secondary role, and perhaps missed out on the primary benefit statistical machine learning can offer.
\item Although, here deep learning can help theorists understand \textit{models} rather than data, which is traditionally focused on.
\item In this work, we've introduced a general methodology that achieves  this goal, and have demonstrated several proofs of concept.
\end{itemize}

\textbf{Theoretical modeling is hard (V1)}
\begin{itemize}
\item Indeed modeling is hard.  Take V1 for example.
\item Maybe say the general thing about how much harder it is to analyze nonlinear ds when going from 2->3->4 here.
\item What we can get from analysis isn't enough.
\item It took EPI to create the insight that produced testable hypotheses of model operation.
\item By flexibly conditioning on different emergent properties, we performed an exploratory analysis of a \textit{model} rather than data.
\end{itemize}

\textbf{Doing EPI at different levels of same stat (SC)}
\begin{itemize}
\item In contrast to the EPI analyses on the V1 model, we conditioned the same model of various levels of the same EP: task performance.
\item This allowed us to identify experimentally testable predictions of changes in connectivity throughout learning.
\end{itemize}
...
\textbf{RNN talk} \\
Recurrent neural networks have become an increasingly popular tool in systems neuroscience research.  The scientific philosophy is as follows: optimize an RNN to execute a task from behavioral neuroscience, compare the activity of this optimized system to brain activity from a model organism doing the same task, and leverage the full observability of the trained RNN to generate hypotheses of the neural mechanisms of computation.  While fixed point identification and jacobian measurement yield intuitive portraits of the implemented computational algorithm, there is dizzying degeneracy in the RNN connectivity matrix with respect to these characterizations.  To develop an understanding of the neural mechanisms at play, we must work in a reduced, interpretable parameter setting of these powerfully general models \cite{doya1993universality}.

In our final analysis, we present a novel procedure for doing statistical inference on interpretable parameterizations of RNNs executing tasks from behavioral neuroscience.  This methodology leverages extensions of DMFT for RNNs developed by Francesca and Srdjan.  I want to say something about opening the Bayesian model selection toolkit for RNNs, but apparently I can't do that :(

\textbf{What do we want to be  the punchline para of discussion?} \\
This is what I was going for before -- 
