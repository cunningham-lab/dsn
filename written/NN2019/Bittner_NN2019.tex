...
An important consideration is the broader contextualization of EPI as a statistical method and its  relation to Bayesian inference. 
...
\end{equation}
The posterior disribution can be analytically calculated when the prior distribution is conjugate with the likehood distribution.
Usually, this is not the case, and alternative methods of approximate Bayesian inference, such as variational inference \cite{blei2017variational}, are utilized.

As we compare EPI to variational inference, it is important to consider that EPI is a maximum entropy method, and that maximum entropy method have a fundamental relationship with exponential family distributions. A maximum entropy distribution of form:
\begin{equation}
\begin{split}
p^*(z) &= \argmax_{p \in \mathcal{P}} H(p(z)) \\
 ...
 \end{split}
\end{equation} 
will have density parameterized by an exponential family:
\begin{equation}
...
Instead we have a normative principle of maximum entropy, and a mathematical specification of an emergent property, which the model must produce.
Thus, we replace the notation of $p(z \mid x)$ with $p(z \mid \mathcal{B})$ conceptualizing an inferred distribution that obeys emergent property $\mathcal{B}$ (see Section \ref{methods_EPI}). 
