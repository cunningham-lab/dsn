...
At a more abstract level of modeling, recurrent neural networks (RNNs) are high-dimensional dynamical models of computation and have become increasingly popular in neuroscience research \cite{barak2017recurrent}. 
...
Such highly parameterized nonlinear dynamical systems are challenging to characterize, let alone probabilistically infer. 
...
Here, the task we consider is gaussian posterior conditioning: calculate the parameters of a gaussian posterior distribution on the mean of a gaussian likelihood $\mu_y$, given a single observation of $y \sim \mathcal{N}(\mu_y,1)$ and a prior $p(\mu_y) = \mathcal{N}(4, 1)$ (Fig. 5A).  The true posterior for an input of $y=2$ is $p(\mu_y \mid y) = \mathcal{N}(3, 0.5)$.  





We used EPI to learn distributions of RNNs producing the correct posterior mean and variance  in their mean activity $\mu=\mu_{\text{post}}$ and temporal variance $\Delta_T=\sigma^2_{\text{post}}$ (respectively), given an input of $y=2$. (see Section \ref{methods_LRRNN}) (Fig. 5B).
