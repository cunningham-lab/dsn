...
Some theoretical work proceeding in that direction extends dynamic mean field theory (DMFT) from random \cite{sompolinsky1988chaos} to low rank RNNs \cite{mastroguiseppe2018linking}.
This theory establishes a link between interpretable, geometric parameterizations of the RNN connectivity with its emerging dynamics.
What's more, one can derive the RNN responses to parameterized inputs, allowing the design of low-rank RNNs to solve behavioral tasks studied in neuroscience like noisy integration and context-dependent decision making (see EPI on such tasks, Fig. S4).
