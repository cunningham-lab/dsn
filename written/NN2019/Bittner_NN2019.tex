...
In the write-up I sent a couple months ago, I suggested that we could model the choices (and neural responses) as Bernoulli variables.  That is, in Pro trials the corresponding Pro neuron responds with a value close to 1 for correct choices, and a value close to 0 for incorrect choices.  Importantly, the Pro neuron in the opposite hemisphere should respond with the opposite value (1 and 0, respectively) on such trials.

It seemed plausible that we could train DSNs to learn distributions of parameterizations of this 4-dimensional dynamical model that produce Bernoulli responses with a given rate and have opposite Left, Right responses in the Pro neurons.  This would need to be checked in practice.
...
\begin{equation}
E_{z_i \sim q_\phi} \left[ E_{\sigma \partial W} \left[ V_{LP,\text{ss}} \mid s=L, c=P, z_i \right] \right] = E_{z_i \sim q_\phi} \left[ \hat{p}_i \right] = p_{LP}
\end{equation}
...
\begin{equation}
 Var_{\sigma \partial W} \left[ V_{LP,\text{ss}} \mid s=L, c=P, z_i \right] = \hat{p}_i(1 - \hat{p}_i)
\end{equation}
...
\begin{equation}
E_{z \sim q\phi} \left[ Var_{\sigma \partial W} \left[ V_{LP,\text{ss}} \mid s=L, c=P, z_i \right] - \hat{p}_i(1 - \hat{p}_i) \right] = 0
\end{equation}
...
\begin{equation}
E_{z \sim q\phi} \left[ E_{\sigma \partial W} \left[ (V_{LP,\text{ss}} - V_{RP,\text{ss}})^2  \mid s=L, c=P, z_i \right] \right] = 0
\end{equation}

Training DSNs to learn distributions of dynamical system parameterizations that produce Bernoulli responses at a given rate (with small variance around that rate) was harder to do than expected.  Long story short: there is a pathology in this optimization setup, where the learned distribution of weights is bimodal attributing a fraction $p$ of the samples to an expansive mode (which always sends $V_{LP}$ to 1), and a fraction $1-p$ to a decaying mode (which always sends $V_{LP}$ to 0).  This pathology was avoided using an inequality constraint prohibiting parameter samples that resulted in low variance of responses across noise.

There are ten total constraints on the DSN visualized in section \ref{section:results}.  Two come from the error increases in the Pro and Anti condition.  We only measure this for left-hemisphere responses, since the right hemisphere is symmetric in the parameterization.  Four constraints come from the Bernoulli variance constraint on the model's responses to the PL, PR, AL, AR conditions.  The remaining four constraints force the responses of the left and right Pro neurons to be opposite for each of those same conditions.

We did not see consistent convergence to a maximum entropy distribution across random initial seeds.  We will focus on this more directly as we come to a well-defined final goal for analysis with DSNs, since that level of diligence is more computationally expensive.
