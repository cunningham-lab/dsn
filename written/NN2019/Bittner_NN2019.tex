...
%[I'd say no to GANs here. MCMC?  Is SMC ABC the deep learning ABC you refer to? Things like sequential neural likelihood (SNL, Leuckman/Papamakarios), LFVI (Dusting) and eventually SNPE will be covered more extensively in appendix, correct?].  
...
First, at a practical level, the nonlinearities and dynamics of many theoretical models are such that conventional inference tools (for example mean field blah blah \cite{???}) typically produce a narrow (though useful) set of insights into these models.  
...
Technically, because many such models stipulate a noisy system of differential equations that can only be sampled or realized through forward simulation, they lack the explicit priors and likelihood terms that are central to the probabilistic modeling toolkit.  

% now we've constructed the tension that theory models need this and statistical models know how to do this... now connect!
To address these three challenges, we developed inference methodology -- the `parameter solution distribution degenerate etc network' -- that provides a distribution over parameter configurations in a theoretical model that give rise to a specified emergent property.  
...
Thus an emergent property is a phenomenon or a feature of the model rather than a single data realization, which is not the central object of interest to the theorist (unlike say the statistical neuroscientist).  
The requirement to condition on an emergent property requires the adaptation of deep probabilistic inference methods, and we adapt recent tools to do so \cite{MEFN}.   % sentence is not quite right but ok for now
Third,  because we can not assume the theoretical model has explicit likelihood on data or the emergent property of interest, we use stochastic gradient techniques in the spirit of likelihood free variational inference \cite{tran}.    
Taken together, the [new DSN name here] network provides a methodology for inferring and then reasoning about parameter configurations that give rise to particular emergent phenomena in theoretical models.   
The [new DSN] network is described schematically in Fig 1A.

Equipped with this methodology, we investigate four models of historical and current importance to the theoretical community.  
These models are chosen to demonstrate a range of complexity (four neuron models up to large recurrent neural networks), a range of neural system (from pattern generators to abstract cortical function), and a range of <something else interesting... scientiifc applicability, mathematical difficulty?>.
First, to clarify the contribution of [new DSN], we investigate network syncing in a classic model of the stomatogastric ganglion \cite{marder}.  
[new DSN] provide a novel insight into how the parameterization of this model can be adjusted with minimal (or maximal) effect to the emergent property of network syncing. 

Second, SC 1-2 sentences
Third, V1 1-2 sentences.
Fourth, LRRNN 1-2 sentences. 
The novel scientific insights offered by [new DSN] contextualize and clarify the previous studies exploring these models and offer a quantitative grounding for theoretical models generally going forward, pointing a way to how rigorous statistical inference can enhance theoretical neuroscience at large.
