...
Duan et al. proposed a four-population dynamical model of superior colliculus with a Pro- and Anti-population in each hemisphere, where activities were bounded from 0-1, and a high output of the Pro population in a given hemisphere corresponds to the  contralateral response. 
...
These consistent Schur eigenvectors have intuitive roles in processing for this task, and are accordingly named the \textit{all}, \textit{side}, \textit{task}, and \textit{diag} modes. The corresponding eigenvalues (e.g. $a_{\text{task}}$ (which change according to $W$) indicate the degree of amplification or supression of activity along that mode.   
...
As learning progresses, the task mode is amplified, indicating the criticality of a distributed task representation at the time of stimulus presentation, (Fig. 4D, purple).  Stepping from task-naive 50\% networks to task-performing 60\% networks, there is a marked suppression of the side mode (Fig. 3D, orange).  Such side mode suppression remains in the regimes of greater accuracy, revealing its importance towards the existence of a distributed task representation.   There were no interesting trends with learning in the all or diag mode. We can conclude that side mode suppression allows rapid task switching, and that greater task-mode representation increases accuracy (Fig. 3E).  

Thus, our EPI-enabled analyses resulted in a clear picture of the connectivity changes that improve accuracy.  Furthermore, we produced an experimentally testable prediction that effective connectvity between these populations changes throughout learning in a way that increases the task mode and decreses the side mode eigenvalues.
