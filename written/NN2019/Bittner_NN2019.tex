...
For EPI in Fig 2C, we used a real NVP architecture with four masks and two layers of 10 units per mask, and batch normalization momentum of 0.99 mapped onto a support of $z \in \left[ \begin{bmatrix} 4 \\ 0 \end{bmatrix}, \begin{bmatrix} 8 \\ 4 \end{bmatrix} \right]$. We used an augmented Lagrangian coefficient of $c_0 = 2$, a batch size $n=300$, set $\nu = 0.1$, and initialized $q_\theta(z)$ to produce a Gaussian with mean $\mu_{\text{init}} = \begin{bmatrix} 6 & 2 \end{bmatrix}$ with standard deviation $\sigma_{\text{init}} = 0.5$.
