...
Recurrent neural networks are abstract, high-dimensional models of computation, which have become increasingly popular in systems neuroscience research \cite{sussillobarakpaper}.  The scientific philosophy is as follows: optimize an RNN to execute a task from behavioral neuroscience, compare the activity of this optimized system to brain activity of a model organism doing the same task, and leverage the full observability of the trained RNN to generate hypotheses of the neural mechanisms of computation.



They are universal approximators of dynamical systems.
The scientific philosophy is as follows: brain produces responses during a task, train an RNN to do the same task.  Does the RNN activity look like the  brain?  Can we infer a neural mechanism from the fully analyzable RNN activity that is experimentally testable.
Emphasize that neurons in RNN aren't really neurons in the brain, just units of a DS.  The latent factors and their evolution are really what we compare to brain activity.
For the most part, this question of linking RNN connectivity to neural mechanism is unanswered.
The high-dimensionality of RNNs is prohibitive to Bayesian model selection/revision practices.

One awesome theoretical work moving us in the right direction is Francesca + Srdjan's work.
