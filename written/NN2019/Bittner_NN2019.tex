...
Secondarily, trained algorithms of varying degrees of biological similarity serve as fully-observable computational systems, that are compared to the brain \cite{ sussillo2013opening, yamins2016using}.  
We believe theoretical neuroscientists are currently focused too heavily on this secondary role, and missing out on the primary benefit statistical machine learning can offer.  
Specifically, deep learning for probabilistic inference has matured to a level in which it can help theorists understand their \textit{models} rather than data for which it is traditionally purposed.
