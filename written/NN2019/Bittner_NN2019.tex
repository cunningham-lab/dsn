...
To continue our running STG example, one such emergent property is the phenomenon of \emph{network syncing} -- in certain parameter regimes, the activity of the hub neuron $x_h$ matches that of the two other populations ($x_{f1},x_{f2}$ and $x_{s1},x_{s2}$).  This emergent property is shown in Figure 1A.
Third, qualtitative parameter analysis ensues: since precise mathematical analysis is intractable in this model, imprecise analysis or perhaps a brute force sweep of parameters is done to describe different parameter configurations that lead to the emergent property.  
