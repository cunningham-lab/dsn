...
However, simply consider DMFT as a black box that takes in a low-rank parameterization $z$ (e.g. $z = \begin{bmatrix} g & M_m & M_n \end{bmatrix}$)  and outputs task-relevant response variables (e.g. average network activity $\mu$, the temporal variability in the network $\Delta_T$, or network activity along a given dimension $\kappa$). 
By recognizing that the equation solver is differentiable, we are able to combine DFMT with EPI to learn distributions on such connectivity parameters of RNNs that execute these tasks via an emergent property defined on the DMFT-produced task-relevant responses.  

(Need to make this more eloquent) 
...
For a toy Bayesian inference problem, the rank-1 RNNs were to produce the posterior mean and variance  in their mean activity $\mu=\mu_{\text{post}}$ and temporal variance $\Delta_T=\sigma^2_{\text{post}}$ (see Section \ref{methods_LRRNN}).  Specifically, given an obervation $y$, the rank-1 RNN had to produce the gaussian posterior of the mean $\mu_y$ of a gaussian likelihood on such observations $y \sim \mathcal{N}(\mu_y, 1)$, and a prior of $\mu_y \sim \mathcal{N}(4, 1)$ (Fig. 4A). The true posterior to this problem for the tested observation $y=2$ is $\mu_y \sim \mathcal{N}(\mu_{\text{post}}=3, \sigma^2_{\text{post}}=0.5)$, although various parameterizations of the connectivity $z = \begin{bmatrix} g & M_m & M_n \end{bmatrix}$ result in approximate inference procedures of varying biases in the resulting $\mu_{\text{post}}$ and $\sigma^2_{\text{post}}$.
